...
export async function generateExcel(opts: GenerateExcelOptions): Promise<GenerateExcelResult> {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      const sheet = pg ? (PAYROLL_SHEETS_MAP[pg] || null) : null
      const sheetName = sheet || 'Unassigned'
    ...
    for (const ws of templateWb.worksheets) {
      ...
      if (WORKSHEET_NAMES.includes(name as typeof WORKSHEET_NAMES[number])) {
        while (ws.rowCount > 0) ws.spliceRows(1, ws.rowCount)
        writePayrollSheet(ws, sheetGroups[name] || [], `${periodLabel} — ${name}`)
      } else if (name === 'Performance Summary') {
        while (ws.rowCount > 0) ws.spliceRows(1, ws.rowCount)
        ws.mergeCells('A1:D1')
        ws.getCell('A1').value = 'Performance Summary'
        ws.getCell('A1').font = { bold: true, size: 12 }
        ws.addRow(['Metric', 'Value', 'Period', periodLabel])

        const totalGross = rows.reduce((s, r) => s + Number(r.grossSalary || 0), 0)
        const totalDed = rows.reduce((s, r) => s + Number(r.totalDeduction || 0), 0)
        const totalNet = rows.reduce((s, r) => s + Number(r.netSalary || 0), 0)
        const totalEmpPen = rows.reduce((s, r) => s + Number(r.employeePension || 0), 0)
        const totalEmprPen = rows.reduce((s, r) => s + Number(r.employerPension || 0), 0)
        const totalTax = rows.reduce((s, r) => s + Number(r.incomeTax || 0), 0)

        ws.addRow(['Total Employees', rows.length])
        ws.addRow(['Gross Salary Total', totalGross])
        ws.addRow(['Total Deductions', totalDed])
        ws.addRow(['Net Pay Total', totalNet])
        ws.addRow(['Employee Pension Total', totalEmpPen])
        ws.addRow(['Employer Pension Total', totalEmprPen])
        ws.addRow(['Income Tax Total', totalTax])
        ws.addRow(['Generated', new Date().toISOString()])
        ws.addRow(['Template Version', TEMPLATE_VERSION])
        ...
        while (ws.rowCount > 0) ws.spliceRows(1, ws.rowCount)
        ws.mergeCells('A1:D1')
        ws.getCell('A1').value = 'Overtime Summary'
        ws.getCell('A1').font = { bold: true, size: 12 }
        ws.addRow(['Employee Code', 'Employee Name', 'Overtime Amount', 'Period'])

        for (const row of rows) {
          const ot = Number(row.overtime || 0)
          if (ot > 0) {
            ws.addRow([row.employeeCode || '', row.employeeName || '', ot, periodLabel])
          }
        }
