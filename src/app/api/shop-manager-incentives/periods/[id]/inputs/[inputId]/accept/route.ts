...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string; inputId: string }> }) {
  try {
    ...
    if (input.inputStatus !== 'SUBMITTED' && input.inputStatus !== 'RETURNED') {
      return badRequest('Only inputs in SUBMITTED or RETURNED status can be accepted')
    }
    ...
    await createAuditLog({
      ...
      action: 'SHOP_MANAGER_INCENTIVE_INPUT_ACCEPT',
      entityType: 'ShopManagerPerformanceInput',
      entityId: inputId,
      oldValue: { inputStatus: oldStatus },
      newValue: { inputStatus: 'ACCEPTED', reviewedById: session.userId, reviewedAt: updated.reviewedAt },
    })
