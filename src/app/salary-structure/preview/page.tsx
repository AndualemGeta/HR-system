...
interface PayRule { id: string; name: string; componentId: string; calculationMethod: string; ruleType: string; [key: string]: unknown }
...
export default function PreviewPage() {
  ...
  const [result, setResult] = useState<{ calculatedAmount: number; explanation: string; warnings: string[] } | null>(null)
    ...
    <div style={{ maxWidth: 600, margin: '0 auto', padding: '2rem 1rem' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginBottom: '1rem' }}>
        ...
        <div>
          ...
          <select value={selectedRule} onChange={e => { setSelectedRule(e.target.value); setResult(null) }} style={{ width: '100%', padding: '0.4rem', border: '1px solid #ccc', borderRadius: 4 }}>
            ...
            {filteredRules.map(r => <option key={r.id} value={r.id}>{r.name} ({r.calculationMethod})</option>)}
          </select>
        </div>
      ...
      {result && (
        <div style={{ background: '#f0fdf4', border: '1px solid #86efac', borderRadius: 6, padding: '1rem' }}>
          ...
          <p style={{ fontSize: '1.25rem', fontWeight: 700, color: '#166534', margin: '0 0 0.5rem' }}>
